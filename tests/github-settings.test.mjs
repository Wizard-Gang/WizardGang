...
test("missing token and read/write permission failures are distinct", async () => {
  await assert.rejects(githubApi("/repos/Wizard-Gang/WizardGang"), { code: "GITHUB_AUTH_REQUIRED" });
  const denied = async () => new Response(JSON.stringify({ message: "denied" }), { status: 403 });
  await assert.rejects(githubApi("/repos/Wizard-Gang/WizardGang", { token: "fixture", fetchImpl: denied }),
    { code: "GITHUB_ADMIN_READ_DENIED" });
  await assert.rejects(githubApi("/repos/Wizard-Gang/WizardGang", { token: "fixture", method: "PATCH", fetchImpl: denied }),
