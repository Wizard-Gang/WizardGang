...
After `release-tag` reconciles an exact tag on merged `main`, it exposes that tag to the dependent `release` job. `release` checks out that exact tag with full Git history, pins the committed Node/npm toolchain, runs `npm ci` and `npm run check`, verifies exact tag/package/commit identity, and publishes with `gh release create --verify-tag`. A rerun verifies an existing non-draft, non-prerelease GitHub Release for the same immutable tag instead of editing, deleting, or replacing it. A deliberate `main` workflow dispatch may recover an existing published release only when the supplied tag resolves to the supplied exact accepted-main commit; it never creates a Release. The release job does not deploy production; WG-088 owns that boundary.
